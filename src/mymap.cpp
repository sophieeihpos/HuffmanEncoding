...
void MyMap::put(int key, int value) {
    ...
    bucketNum=hashCode(key) % nBuckets;


    key_val_pair* cp=buckets[bucketNum];
    key_val_pair* previous=nullptr;
    while(true){
        if(cp==nullptr){
            cp=new key_val_pair;
            cp->key=key;
...
int MyMap::get(int key) const {
    ...
    int bucketNum=hashCode(key) % nBuckets;
    key_val_pair* cp=buckets[bucketNum];
    while(cp!=nullptr){
        if(key==cp->key){
            return cp->value;
    ...
    throw ErrorException("The key is not in the map!");
}
